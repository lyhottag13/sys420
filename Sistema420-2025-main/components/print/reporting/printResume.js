...
const GeneratePdf = ({ testsArray, totals }) => {
  ...
  */
  ...
  async function generateExcel() {
    ...
    const singleTest = options.selected_tests;
    const { headers, data } = getRawDataTable(singleTest);
    const worksheet = workbook.addWorksheet(shortenFilenames(singleTest));
  ...
  saveAs(blob, `${shortenFilenames(singleTest)}.xlsx`);
}
...
/**
 * Shortens the filename to a format that won't trail off pages or exceed character limits.
 * @param {object} test The test whose filenames will be shortened
 * @returns A shortened description of the filenames
 */
  ...
  <div className="w-full bg-white">
    ...
    }
    {
      ...
      <div ref={paretoChartRef}>
        {
            <div key={`pareto-container-test`} style={{ maxWidth: "none", maxHeight: "none", height: "600px", width: "600px" }}>
              <ParetoChart selectedTest={options.selected_tests} printing={true} />
            </div>
        }
      </div>
    }
    {
      ...
      <div ref={histogramsContainerRef}>
        ...
        {
            <div key={`histogram-main-container-test`} style={{ maxWidth: "none", maxHeight: "none", height: "270px", width: "600px" }}>
              {<MainHistogramContainer hideFails={!options.include_fails} selectedTest={!options.highlighted_tests_types ? options.selected_tests : options.highlighted_tests_types} printing={true} />}
            </div>
        }
      </div>
    }
